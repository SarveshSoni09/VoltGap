...
| ID | Assumption | Status | Evidence |
|---|---|---|---|
| A-2.1 | The 4 DBSCAN clusters over 200 m diameter do not materially distort candidate siting | **RESOLVED BY MEASUREMENT.** The first attempt argued from the ratio 500 m / 3,834 m; external review rejected that, correctly — a ratio shows clusters are small relative to cells and shows nothing about whether one straddling a boundary flips a classification. The argument is **withdrawn** and replaced by a three-condition experiment per state (shipped/no clustering, DBSCAN eps 50 m, DBSCAN eps 200 m), scoring candidate-set Jaccard, saturation reclassifications, portfolio overlap at 5/20/50 sites and the demand and equity objective deltas. At the shipped Phase 1 configuration (**eps 50 m**) the effect is **exactly zero in all six states**. At a deliberately coarser **200 m**, Washington reclassifies 1 cell (Jaccard 0.998519) and Texas 2 (0.999173), and **no portfolio changes at any budget in any state** — overlap 1.000 throughout, demand and equity deltas exactly 0.00 | `test_p4_f_a21_site_clustering_was_measured`, `test_p4_f_a21_the_shipped_clustering_configuration_changes_nothing`, `test_p4_f_a21_no_portfolio_changes_under_any_clustering_condition` |
...
| ID | Falsifiable statement | Depends on | Tested in | Status |
|---|---|---|---|---|
| A-4.1 | ~~A resident-population filter is an adequate stand-in for the road-network proximity filter §7.8 specifies.~~ | **WITHDRAWN AND RESOLVED 2026-08-31.** The assumption rested on a false premise — that no road-network dataset was available. TIGER/Line publishes primary and secondary roads per state, free and keyless. The §7.8 filter now runs on TIGER/Line 2024 PRISECROADS (MTFCC S1100 + S1200) at 5.0 km, and removes 182 candidates in Washington, 54 in Tennessee, 103 in Montana, 8 in Vermont, 656 in Texas and 358 in California. The resident-population filter is retained **on its own merits** — a cell with nobody in it is not a siting candidate — and is no longer described as standing in for anything | Resolved at the Phase 4 gate | **CLOSED** |
| A-4.6 | ~~Distance from a cell centroid to the nearest road **vertex** is close enough to distance to the nearest point on a road **segment**.~~ | **WITHDRAWN AND RESOLVED 2026-08-31.** Deferring this was wrong: it is an error in a hard candidate filter, where an overestimate does not degrade a number, it removes a cell from consideration. The method was replaced rather than the assumption confirmed. Distance is now measured to the nearest **point on** the nearest segment (`PolylineIndex`), and is a true great-circle distance to a real location on a road. Measured across all six frontier states: **zero** cells change side of the 5.0 km threshold, candidate Jaccard **1.000000** everywhere, every portfolio identical at 5, 20 and 50 sites; largest distance error 666.9 m (Texas), 34 cells overstated by more than 100 m in total. Impact-log entry **I-20** | Resolved at the Phase 4 gate | **CLOSED** |
| A-4.8 | ~~The nearest point along a segment, located in a tangent plane centred on the query point, is the same point the exact spherical calculation would pick.~~ | **VALIDATED AND CLOSED 2026-08-31.** Measured, not argued. Both methods parameterise the same curve — `lerp(A, B, t)` in (lat, lon) — so only the choice of `t` was under test; the reference minimises true haversine over `t` by golden-section search with no projection anywhere. Across 200 cells in each of the six frontier states: worst error **17.5 mm** (Montana), p99 **2.5 mm**, mean **0.18 mm**, against a **5 km** threshold. **Zero** cells reclassified at the threshold in any state, and zero negative errors. The worst adversarial case constructible inside the regime the method is used in — a 6-degree-span segment at 78°N, ~15 km away — errs by **95 mm**. Impact-log entry **I-20** | Resolved at the Phase 4 gate | **CLOSED** |
...
## Opened by Phase 5 (2026-08-31)

| ID | Falsifiable statement | Depends on | Tested in | Status |
|---|---|---|---|---|
| A-5.1 | The AFDC annual state registration page for year Y was available in January Y+1, so a Y vintage is legitimately usable at a Y+1 cutoff. | Assumed from the page being labelled for a year, not verified against a dated capture. Assumption **A-0.5** sits underneath it: Phase 1 established the pages are *stable* across 2022-2026 but not that they are *contemporaneous*, because no capture predates 2022-08-18. If AFDC reconstructed the annual series retrospectively from later VIN data, every historical origin's reconciliation constraint carries information that did not exist at its cutoff. | Phase 6, or whenever an archival capture predating 2022-08-18 is located | OPEN |
| A-5.2 | A Poisson fit on 51 state-level observations is a fair reconstruction of the model a person could have built at each cutoff. | It is *a* defensible reconstruction, not the only one. No sub-state registration data is retrievable for 2019-2021, so a panel fit is impossible; but a modeller in 2020 might have had proprietary or state-agency data this project cannot obtain. The backtest therefore measures a *lower bound* on what was buildable, and §10.2.3's "the backtested model and the deployed model are not the same model" is stated in full. | Phase 6 | OPEN |
| A-5.3 | Scoring the backtest on H3 cells makes the 2010/2020 tract-geography break immaterial to the ranking metrics. | Argued from the fact that H3 cells do not move when the Census redraws tracts, and that every §10.2.4 metric is a cell-ranking metric. **Not measured** against a 2020-geography backtest. 2020 tract geography was not contemporaneously available for these prediction cutoffs; retrospective harmonization would require later boundary/crosswalk information, so Core evaluates stable H3 cells using cutoff-appropriate 2010-geography inputs. A harmonised comparison is possible in principle using a later crosswalk, but that crosswalk is itself post-cutoff information, so it would answer a different question than the one D1 permits here. | Phase 6, or whenever a 2020-geography historical release becomes relevant | OPEN |
| A-5.4 | ~~The ACS 2020 5-year release postdates 2022-01-01, so the 2022 origin may not use it.~~ | **CLOSED 2026-08-31 on external review.** The 2016-2020 ACS 5-year estimates were released **2022-03-17**, which is after the cutoff, so the assumption held and vintage selection is unchanged: the origins still resolve to ACS 2018 / 2019 / 2019. `release_date_certain` is now true, and the edition is excluded because it demonstrably did not exist at the cutoff rather than because its availability could not be shown. The conservative "resolve toward the older vintage" fallback remains in the code for any vintage whose date is genuinely unestablished | Resolved at the Phase 5 gate | **CLOSED** |
| A-5.5 | A random baseline estimated from ONE seeded permutation is a fair comparator. | **WITHDRAWN AND RESOLVED 2026-08-31.** It is unbiased but high-variance: deployment counts are heavily concentrated, so a single shuffle either does or does not land on the busy cells. Over 400 draws at the 2020 origin the top-decile capture has mean 0.0976 and standard deviation 0.0137, and the shipped single-seed draw of 0.0687 sat at **percentile 0**, below the 5th percentile of 0.0759 - which overstated lift there as 9.40x. The baseline is now the mean over 200 draws, still empirical rather than theoretical, with its spread published. Lift vs random becomes 6.72 / 6.39 / 5.89, and lift vs population is unchanged at 0.83 / 0.82 / 0.81. Impact-log entry **I-23** | Resolved at the Phase 5 gate | **CLOSED** |

**Re-checked at the Phase 5 gate.** A-0.5 remains **OPEN** and its limitation is stated in
