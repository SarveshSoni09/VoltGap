...
| A-4.6 | Distance from a cell centroid to the nearest road **vertex** is close enough to distance to the nearest point on a road **segment**. | A long straight segment between two distant vertices could pass closer to a cell than either endpoint, so vertex distance is an upper bound on true distance and the filter is therefore marginally over-exclusive. TIGER geometries are densely vertexed (3,006 Washington features carry 376,007 vertices, ~125 each), so the error should be small — but it is unmeasured. Measuring it needs point-to-segment distance, which needs a geometry dependency the zero-cost offline build has so far avoided. | Phase 6, or whenever a geometry dependency is added | OPEN |
