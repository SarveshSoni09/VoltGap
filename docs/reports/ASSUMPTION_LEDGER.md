...
| A-5.3 | Scoring the backtest on H3 cells makes the 2010/2020 tract-geography break immaterial to the ranking metrics. | Argued from the fact that H3 cells do not move when the Census redraws tracts, and that every §10.2.4 metric is a cell-ranking metric. **Not measured** against a 2020-geography backtest, because one cannot be built at these cutoffs: every ACS release available before 2022-01-01 is on 2010 boundaries. | Phase 6, or whenever a 2020-geography historical release becomes relevant | OPEN |
| A-5.4 | The ACS 2020 5-year release postdates 2022-01-01, so the 2022 origin may not use it. | Widely reported to have slipped from December 2021 to March 2022 for pandemic collection reasons, but **not verified against a primary source by this project**. Declared `release_date_certain: false`, so the guard falls back to ACS 2019 — the direction that cannot manufacture leakage. The cost is that the 2022 origin uses features two years older than it might legitimately have been allowed. | whenever the release date is verified | OPEN |
