...
| A-4.8 | The nearest point along a segment, located in a tangent plane centred on the query point, is the same point the exact spherical calculation would pick. | The projection distorts by O(d³/R²) over the tens of kilometres involved, which is millimetres. It can only move *which* point along the segment is chosen; the distance finally reported for that point is measured with haversine on the sphere, so the reported value is a genuine geodesic distance to a real location on a road regardless — and therefore a rigorous upper bound on the true minimum. What is unproven is that no *other* point on the segment is closer still, by an amount the projection hid. | Phase 6, or whenever a geodesic geometry dependency is added | OPEN |
| A-4.7 | Primary and secondary roads (MTFCC S1100 + S1200) are the right road classes for a public charging siting filter. | Pre-registered before any result was recomputed, on the reasoning that at 38.2 km² per cell nearly every inhabited cell contains some local street, so including S1400 would make the filter a near no-op. But a public charging site does not strictly require an arterial, and a cell served only by local streets is excluded even though something could physically be built there. The 1–20 km sensitivity curve is published; the *road-class* choice has no equivalent sensitivity. | Phase 6 | OPEN |
